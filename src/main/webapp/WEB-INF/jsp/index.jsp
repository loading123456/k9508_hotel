			...
			<c:forEach items="${listMap}" var="dataMap">
				<li class="layui-nav-item">
					<a href="javascript:;"><i class="layui-icon">
						<c:if test="${dataMap['pId']==1}">&#xe63c;</c:if>
						<c:if test="${dataMap['pId']==2}">&#xe62a;</c:if>
						<c:if test="${dataMap['pId']==3}">&#xe770;</c:if>
						<c:if test="${dataMap['pId']==4}">&#xe68e;</c:if>
						<c:if test="${dataMap['pId']==5}">&#xe716;</c:if>
						<c:if test="${dataMap['pId']==6}">&#xe63a;</c:if>
					</i>${dataMap['pName']}</a>
					<dl class="layui-nav-child" style="padding-left: 15px">
						<c:forEach items="${dataMap['secondAuths']}" var="secondAuth">
							<dd>
						...
						<div carousel-item style="height: 750px;">
							<div>
								<div class="layui-bg-green demo-carousel">
									<p style="font-size: 30px;">xx集团酒店管理系统</p>
